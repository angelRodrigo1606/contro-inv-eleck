<div class="space-y-4">
    ...
    <div>
        ...
        @if(isset($product))
            <p class="text-sm text-theme-text-muted mt-1">El SKU no se puede modificar.</p>
        @endif
    </div>

    <div>
        <x-input-label for="description" :value="__('Descripción')" />
        <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-theme-input-border focus:border-brand-primary focus:ring-brand-primary rounded-md shadow-sm bg-theme-surface text-theme-text">{{ old('description', $product->description ?? '') }}</textarea>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <select id="category_id" name="category_id" class="mt-1 block w-full border-theme-input-border focus:border-brand-primary focus:ring-brand-primary rounded-md shadow-sm bg-theme-surface text-theme-text" required>
        ...
        <div>
            ...
            <select id="supplier_id" name="supplier_id" class="mt-1 block w-full border-theme-input-border focus:border-brand-primary focus:ring-brand-primary rounded-md shadow-sm bg-theme-surface text-theme-text" required>
...
<div class="flex items-center gap-4 mt-6">
    ...
    <a href="{{ route('products.index') }}" class="text-sm text-theme-text-muted hover:text-theme-text">Cancelar</a>
</div>
