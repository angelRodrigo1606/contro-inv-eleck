                ...
                <div class="p-6 text-theme-text">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="space-x-2">
                            ...
                            <a href="{{ route('reports.entries.export', ['format' => 'pdf'] + request()->all()) }}" class="inline-flex items-center px-4 py-2 bg-brand-secondary border border-transparent rounded-md font-semibold text-xs text-theme-text uppercase tracking-widest hover:bg-brand-secondary/80 transition-colors">PDF</a>
                        </div>
                    </div>
                        ...
                        <tbody class="bg-theme-surface divide-y divide-theme-border">
                            @foreach($movements as $movement)
                                <tr class="hover:bg-theme-surface-alt transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-theme-text-muted">{{ $movement->product?->category?->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-theme-text-muted">{{ $movement->quantity }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-theme-text-muted">{{ $movement->product?->supplier?->name }}</td>
